...
class SuperNet(tf.Module):
  ...
  def __init__(self, n_filters, n_dense, n_classes):
    self.conv_3x3_1 = L.Conv2D(n_filters, (3, 3), padding='same', activation='relu', name='3x3_1_conv_subnets_1_and_2')
    self.conv_3x3_2 = L.Conv2D(n_filters, (3, 3), padding='same', activation='relu', name='3x3_2_conv_subnets_1_and_3')
    self.conv_5x5_1 = L.Conv2D(n_filters, (5, 5), padding='same', activation='relu', name='5x5_1_conv_subnets_3_and_4')
    self.conv_5x5_2 = L.Conv2D(n_filters, (5, 5), padding='same', activation='relu', name='5x5_2_conv_subnets_2_and_4')
    ...
    # self.subnets = {f'subnet_{i}': self._create_subnet(i) for i in [0, 1, 2, 3, 4]}

  def _sample_subnet(self, subnet_idx, path_dropout_prob=0):
    ''' Assemble one of 4 avaliable sub networks from shared layers.
        `subnet_idx` - index of specific sub network int from 0 to 4.
          0 - corresponds to SuperNet
          1-4 - corresponds to SubNets

        returns:
          model with acritecture corresponds to `subnet_idx` 
    '''
  ...
  def _train_random_subnet(self, X, y, optimizer, model_idx):
    model = self._sample_subnet(model_idx)
    loss_value, grads = self.grads(model, X, y)
    optimizer.apply_gradients(zip(grads, model.trainable_variables))
    return loss_value
      
  def _train_full(self, X, y, optimizer):
    model = self._sample_subnet(0)
    loss_value, grads = self.grads(model, X, y)
    optimizer.apply_gradients(zip(grads, model.trainable_variables))
    return loss_value

  def _train_with_path_dropout(self, X, y, optimizer, path_dropout_prob):
    model = self._sample_subnet(0, path_dropout_prob)
    loss_value, grads = self.grads(model, X, y)
    optimizer.apply_gradients(zip(grads, model.trainable_variables))
    return loss_value
  ...
  def eval_supernet(self, validation_data):
    ...
    supernet = self._sample_subnet(0)
  ...
  def fit(self, X_train, y_train,
          n_epoches, batch_size, n_btch_to_switch_subnet, learning_rate,
          validation_data, path_dropout_prob=0, train_mode='full', verbosity=4):
    
    ''' Train SuperNet with one of defined strategies:
            'full' - trains full SuperNet end-to-end.
            'random_subnet' - randomly choose one of SubNets (icluding SuperNet)
                              train on it and choose another SubNet after every 
                              `n_btch_to_switch_subnet` batches for every epoch.          
            'path_dropout' - randomly zeroing one of convolution layers after every 
                              `n_btch_to_drop` batches for every epoch. 
    ...
    for epoch in range(n_epoches):
      ...
      for i, (X, y) in enumerate(self._batch_gen(X_train, y_train, batch_size)):
        ...
        if train_mode == 'full':
          loss_value = self._train_full(X, y, optimizer)

        elif train_mode == 'random_subnet':
      ...
      if epoch % verbosity == 0:
        print(f'Epoch: {epoch}, SuperNet_train_loss: {epoch_loss.result():.4f}, SuperNet_train_accuracy: {epoch_accuracy.result():.2%}')
        if validation_data:
          supernet_validation_accuracy = self.eval_supernet(validation_data)
          print(f'Epoch: {epoch}')
          print(f'  SuperNet validation accuracy: {supernet_validation_accuracy:.3%}')
